...
class TrainingStatsTracker():
    def __init__(self, window_size: int = 100):
        ...
        logger.info(
            f"TrainingStatsTracker initialized with "
    ...
    def end_epoch(self, final_score: int, epsilon: float = None):
        ...
        logger.info(
            f"Epoch {current_epoch}: Total Reward={total_reward:.2f}, "
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'avg_score_recent': (
                self.avg_scores[-1] if self.avg_scores else 0
            ),
            'avg_reward_recent': (
                self.avg_rewards[-1] if self.avg_rewards else 0
            ),
            'max_survival': max(self.epoch_steps),
            'avg_survival_recent': (
                self.avg_steps[-1] if self.avg_steps else 0
            ),
    ...
    def save_plots(self, filepath: str,
        ...
        ax1.plot(epochs, self.epoch_rewards, alpha=0.3, color='blue',
                 label='Total Reward', linewidth=1)
        ax1.plot(epochs, self.avg_rewards, color='blue', linewidth=2,
                 label=f'Moving Avg ({self.window_size})')
        ...
        ax2.plot(epochs, self.epoch_scores, alpha=0.3, color='green',
                 label='Score', linewidth=1)
        ax2.plot(epochs, self.avg_scores, color='green', linewidth=2,
                 label=f'Moving Avg ({self.window_size})')
        ax2.set_xlabel('Epoch', fontsize=12)
        ax2.set_ylabel('Score', fontsize=12)
        ax2.set_title('Average Score Over Time', fontsize=14,
                      fontweight='bold')
        ax2.legend(loc='best')
        ...
        ax3.plot(epochs, self.epoch_steps, alpha=0.3, color='orange',
                 label='Steps', linewidth=1)
        ax3.plot(epochs, self.avg_steps, color='orange', linewidth=2,
                 label=f'Moving Avg ({self.window_size})')
        ax3.set_xlabel('Epoch', fontsize=12)
        ax3.set_ylabel('Steps (Lifetime)', fontsize=12)
        ax3.set_title('Average Lifetime Over Time', fontsize=14,
                      fontweight='bold')
        ax3.legend(loc='best')
...
Latest Epoch:
  ...
  • Steps: {self.epoch_steps[-1]}
        ...
        if has_epsilon:
            ...
            ax5.plot(epsilon_epochs, self.epoch_epsilons,
                     color='purple', linewidth=2, label='Epsilon (ε)')
            ax5.set_xlabel('Epoch', fontsize=12)
            ax5.set_ylabel('Epsilon Value', fontsize=12)
            ax5.set_title('Epsilon Decay Over Time', fontsize=14,
                          fontweight='bold')
            ax5.legend(loc='best')
